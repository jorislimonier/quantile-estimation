...
for sheet_name in sheet_names:
    for file_name in file_names:
        ...
        data_vis.plot_histogram(data_prep).write_image(f"fig/data/{data_prep.protocol}_{data_prep.sim_time}.png", scale=5)

        for k in range(3,6):
            print(f"{sheet_name} -- {file_name} -- quantile {k} -- {len(results)}")
            boot = Bootstrap(k=k, results=results, confidence_level=.95)
